...
async function connectToWhatsApp() {
  ...
  if (!sock.authState.creds.registered) {
    setTimeout(async () => {
      try {
        ...
        console.log(`\n\n🟢 CODIGO DE VINCULACIÓN: ${codeLimpio} 🟢\n\n`);
      } catch (e) { 
        console.log("⚠️ Esperando conexión...", e.message); 
      }
    }, 3000);
  }
  ...
  sock.ev.on('messages.upsert', async ({ messages }) => {
    try {
      ...
      if (command === '.adm') {
        await sock.sendMessage(remoteJid, { text: "👑 *ADMINISTRADORES*\n👤 Uvi - +525654812179\n👤 Estef - +573114860414\n👤 Samu - +573173607093\n👤 Cham - +59894793177\n👤 Ore - +50687309582" });
      }

      // Lógica de Salas ARAM
      if (command === '.aram') {
        ...
        if(text.includes('avisar') && remoteJid.endsWith('@g.us')) {
          ...
          ments = meta.participants.map(p=>p.id);
          aviso = "\n📢 *LLAMADO GENERAL*";
        }
        ...
        lobbies[remoteJid][salaID] = {
          ...
          timer: setTimeout(() => {
            ...
            sock.sendMessage(remoteJid,{text:`🗑️ Sala ${salaID} de ARAM ha expirado.`})
          }, 300000)
        };

        await sock.sendMessage(remoteJid, {
          text: `🎮 *ARAM* (Sala ${salaID})\n👥 1/${limite}\n👉 Escribe *.me uno ${salaID}* para unirte.${aviso}`, 
          mentions: ments
        });
      }

      // Lógica de Salas RANKED
      if (command === '.ranked') {
        ...
        lobbies[remoteJid][salaID] = {
          ...
          timer: setTimeout(() => {
            delete lobbies[remoteJid][salaID]; 
            sock.sendMessage(remoteJid, {text: `🗑️ Sala ${salaID} de Ranked ha expirado.`})
          }, 300000)
        };

        await sock.sendMessage(remoteJid, {
          text: `🎮 *RANKED* (Sala ${salaID})\n🏅 ${rango}\n👥 1/${limite}\n👉 Escribe *.me uno ${salaID}* para unirte.${aviso}`, 
          mentions: ments
        });
      }
      ...
      if (command === '.me' && subCommand === 'uno') {
        ...
        if(sala.participantes.length < sala.limite){
          let l = "";
          sala.participantes.forEach((p,i) => l += `\n${i+1}. @${p.split('@')[0]}`);
          await sock.sendMessage(remoteJid, {
      ...
      if (command === '.encuesta') {
        ...
        let p = c.split('/').map(s=>s.trim()).filter(s=>s);
