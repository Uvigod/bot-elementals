    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        if (command === '.adm') {
            if (subCommand === 'nuevo' || subCommand === 'add') {
                ...
                let nuevoNombre = args[2]; let nuevoNumero = args[3];
                if (!nuevoNombre || !nuevoNumero) { await sock.sendMessage(remoteJid, { text: "⚠️ Uso: .adm nuevo [Nombre] [Numero]" }); return; }
            ...
            if (subCommand === 'borrar' || subCommand === 'del') {
                if (!sender.includes(MY_PHONE_NUMBER)) { await sock.sendMessage(remoteJid, { text: "⛔ Acceso denegado." }); return; }
                let nombreBorrar = args[2];
                if (!nombreBorrar) { await sock.sendMessage(remoteJid, { text: "⚠️ Uso: .adm borrar [Nombre]" }); return; }
                let largoInicial = admins.length;
                admins = admins.filter(a => a.nombre.toLowerCase() !== nombreBorrar.toLowerCase());
                if (admins.length < largoInicial) await sock.sendMessage(remoteJid, { text: `🗑️ Admin Eliminado: ${nombreBorrar}` });
                else await sock.sendMessage(remoteJid, { text: `⚠️ No encontré a "${nombreBorrar}".` });
                return;
            }
        ...
        if (command === '.aram') {
            ...
            if (subCommand === 'duo') { limite = 2; etiquetaModo = "DUO"; } else if (subCommand === 'trio') { limite = 3; etiquetaModo = "TRIO"; } else if (subCommand === 'cuarteto' || subCommand === '4q') { limite = 4; etiquetaModo = "CUARTETO"; } else if (subCommand === '5q') { limite = 5; etiquetaModo = "5Q"; }
            let mentions = [sender]; let avisoTexto = "";
            if (args.includes('avisar') || args.includes('todos')) { if (remoteJid.endsWith('@g.us')) { try { const groupMetadata = await sock.groupMetadata(remoteJid); mentions = groupMetadata.participants.map(p => p.id); avisoTexto = "\n📢 *¡LLAMADO GENERAL!*"; } catch (e) { } } }
            ...
            lobbies[remoteJid][salaID] = { id: salaID, tipo: `ARAM ${etiquetaModo} ❄️`, rango: 'Abismo de los Lamentos', limite: limite, participantes: [sender], timer: setTimeout(() => { if (lobbies[remoteJid][salaID]) { sock.sendMessage(remoteJid, { text: `⏰ Sala #${salaID} cerrada.` }); delete lobbies[remoteJid][salaID]; } }, 5 * 60 * 1000) };
        ...
        if (command === '.ranked') {
            ...
            let salaID = null; for (let i = 1; i <= MAX_LOBBIES; i++) { if (!lobbies[remoteJid][i]) { salaID = i; break; } }
            if (!salaID) { await sock.sendMessage(remoteJid, { text: `⚠️ Salas llenas.` }); return; }
            let limite = (subCommand === 'duo') ? 2 : (subCommand === 'trio') ? 3 : 5; let tipo = subCommand.toUpperCase(); let argumentosLimpios = eloArg.replace('avisar', '').replace('todos', '').trim(); let nombreRango = RANGOS[argumentosLimpios] || "Elo Libre";
            lobbies[remoteJid][salaID] = { id: salaID, tipo: `RANKED ${tipo}`, rango: nombreRango, limite: limite, participantes: [sender], timer: setTimeout(() => { if (lobbies[remoteJid][salaID]) { sock.sendMessage(remoteJid, { text: `⏰ Sala #${salaID} cerrada.` }); delete lobbies[remoteJid][salaID]; } }, 5 * 60 * 1000) };
        ...
        if (command === '.me' && subCommand === 'uno') {
            ...
            if (salasActivas.length === 1) { idTarget = salasActivas[0]; } else { if (argNumero && lobbies[remoteJid][argNumero]) { idTarget = argNumero; } else { await sock.sendMessage(remoteJid, { text: `⚠️ Usa: *.me uno ${salasActivas.join('*, *.me uno ')}*` }); return; } }
            const sala = lobbies[remoteJid][idTarget]; if (sala.participantes.includes(sender)) return; sala.participantes.push(sender);
            if (sala.participantes.length < sala.limite) { let lista = ""; sala.participantes.forEach((p, i) => { lista += `\n${i + 1}. @${p.split('@')[0]}`; }); const msj = `🎮 *${sala.tipo}* (Sala #${sala.id})\n🏅 *${sala.rango}*\n👥 Cupos: ${sala.participantes.length}/${sala.limite}\n────────────────${lista}\n────────────────\n👉 Escribe *.me uno ${sala.id}*`; await sock.sendMessage(remoteJid, { text: msj, mentions: sala.participantes }); } 
            else { clearTimeout(sala.timer); let txt = `🚀 *TEAM COMPLETO (Sala #${sala.id})*\n🎮 *${sala.tipo}*\n🏅 *${sala.rango}*\n\n`; sala.participantes.forEach((p, i) => txt += `${i+1}. @${p.split('@')[0]}\n`); txt += `\n#ELNS`; await sock.sendMessage(remoteJid, { text: txt, mentions: sala.participantes }); delete lobbies[remoteJid][idTarget]; }
        }
        if (command === '.encuesta') { let contenido = text.replace(/^\.encuesta\s*/i, '').trim(); if (!contenido) { await sock.sendMessage(remoteJid, { text: "⚠️ .encuesta Pregunta / Op1 / Op2" }); return; } let partes = contenido.split('/').map(s => s.trim()).filter(s => s); let opciones = partes.length > 1 ? partes.slice(1) : ["Sí", "No"]; await sock.sendMessage(remoteJid, { poll: { name: "📊 ELNS: " + partes[0], values: opciones, selectableCount: 1 } }); }
